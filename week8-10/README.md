...
| **Zip Code Heatmaps** | Zip-code heat maps of median close price and homes sold, filterable by month + the four dimensions | Two maps on Tableau's basemap (zips geocoded by Tableau; one circle per zip, color = the measure, size = homes sold, color legends) with Month / Zip / Property Type / County / City dropdowns — plus a **Brokerage** dropdown that turns either map into a footprint map for any brand (the capstone's geographic lens: where does Compass sell, and at what price?) |
