...
FIELDNAMES = [
    ...
    'SubdivisionName', 'BuyerOfficeAOR', 'YearBuilt', 'StreetNumberNumeric', 'ListingId',
    ...
    'NewConstructionYN', 'GarageSpaces', 'HighSchoolDistrict', 'PostalCode', 'AssociationFee',
...
def extract_month(yyyymm, token):
    """Pull all closed-sale records for one YYYYMM into CRMLSSold<YYYYMM>.csv.
    Returns the (possibly refreshed) token."""
    start, end = parse_yyyymm(yyyymm)
    csv_file = f'CRMLSSold{yyyymm}.csv'
    ...
    params = {
        ...
        '$filter': (
            ...
            f"CloseDate ge {start.isoformat(timespec='milliseconds')}Z and "
            f"CloseDate lt {end.isoformat(timespec='milliseconds')}Z"
        ),
        '$top': 1000,
    }
