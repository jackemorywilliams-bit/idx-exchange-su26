...
**The competitive workbook (Weeks 9–10), built the same way.** `make_competitive_extracts.py` resolves entities the way the program's case study prescribes (normalized office keys, agent = name + office, 255 placeholder records excluded from every ranking) and writes three extracts; `make_competitive_workbook.py` generates seven tabs: the top-100 listing agents and offices by units and by volume as ranked tables with the director's columns (Rank · Agent, Office · Units Sold · Sales Volume · Market Share %), two zip-code heat maps (zip × month color grids for the 60 busiest zips, filterable by month and the four dimensions), and the own-design **Brokerage Power** tab — Compass share of listing sides 7.9% in 2026 H1 and still rising, Keller Williams and RE/MAX sliding, Real Broker and Equity Union doubling, and Opendoor's listing activity collapsing from 109 a month to 14. Details and the tab table are in [week8-10/README.md](week8-10/README.md).
